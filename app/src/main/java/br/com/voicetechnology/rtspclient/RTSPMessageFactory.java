...
public class RTSPMessageFactory implements MessageFactory {
    ...
    private static Map<Method, Class<? extends Request>> requestMap;

    static {
        headerMap = new HashMap<String, Constructor<? extends Header>>();
        requestMap = new HashMap<Method, Class<? extends Request>>();
    ...
    public void incomingMessage(MessageBuffer buffer)
        ...
        try {
            ...
            while (true) {
                ...
                if (line.length() == 0)
                    break;
                Constructor<? extends Header> c = headerMap.get(line.substring(0, line.indexOf(':')).toLowerCase());
                if (c != null)
            ...
            try {
                ...
                if (in.available() < length)
                    throw new IncompleteMessageException("in.available() < length");
            ...
            } catch (MissingHeaderException ignored) {
            }
